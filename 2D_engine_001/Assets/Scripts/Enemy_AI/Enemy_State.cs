...
public class Enemy_State : MonoBehaviour {
	...
	{
		if (SWORD.gameObject.tag == "PlayerAttacks") {
			...
			if (!(this.gameObject.tag == "Boss")) {
				...
				recentdamagedealt = recentdamagedealt + SWORD.GetComponent<Sword_State> ().dmg - resistance;
				if (recentdamagedealt > 200) {
					recentdamagedealt = 0;
					Rigidbody2D rb = SWORD.GetComponentInParent<Rigidbody2D> ();
